...
for(i in 1: length(soci1))
  ...
  values(raster_costO)[values(raster_costO)>=0.099262461]<-NA # 5% q
  ...
  {print(paste("rtoP",print(Sys.time())))
    ...
    if(length(mat[polO,][mat[polO,]%in%1])>1) 
    {
      ...
      save(unifiedPolygons2, file=paste0("data/Re_analysisGEbarriers/P2_datamodel/peopleinspaceTemp5QT/", soci1[i], ".rds"))
      } # from mtargetgr
    } # from nonb
...
save(nonb, file="ata/Re_analysisGEbarriers/P2_datamodel/peopleinspaceTemp5QT/nonb.rds")
save(multiple0, file="data/Re_analysisGEbarriers/P2_datamodel/peopleinspaceTemp5QT/multiple0.rds")
save(mtargetgr, file="data/Re_analysisGEbarriers/P2_datamodel/peopleinspaceTemp5QT/mtargetgr.rds")
