...
load(file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/","listw",".rds"))
load(file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/","nbhoodV210","/","pcaik_","_", traits_of_interest[j],".rds"))
...
for(k in 1: 2)
  ...
  for(j in 1: length(traits_of_interest))
    if(!(k==2 & j==34)) # errors, no convergence
      { trait_of_interest<-traits_of_interest[j]
      if(!paste(trait_of_interest, name_folder)%in%listw & paste0("dfmS2_", "_",trait_of_interest,".csv") %in% list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/",name_folder)))  
        # for now the ones that 100% work
        {read.csv(file =paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/",name_folder,"/","dfmS2_", "_",trait_of_interest,".csv"), stringsAsFactors = F)->dfm
          ...
          save(model, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/models/",name_folder,"/model_",trait_of_interest,"_",100,".rds"))
          ...
          df_out<-data.frame("Trait"=trait_of_interest, "Name_tr"=vars[vars$id%in%trait_of_interest,]$title, 
              ...
              save(df_out, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folder,"/model_",trait_of_interest,".rds"))
              
       # replace the names of RCs with real predictor name
       load(file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/",name_folder,"/","RCtraits_","_", trait_of_interest,".rds")) # RCtraits
       RCs<-character()
       ...
       save(df_out, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folder,"/dfoutNamesPred_",trait_of_interest,".rds"))
...
m1<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folders[1],"/"))
m1<-m1[grep("dfoutNamesPred", m1)]
m2<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folders[2],"/"))
m2<-m2[grep("dfoutNamesPred", m2)]

rm(dfm1)
for(i in 1:length(m1))
  { load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folders[1],"/",m1[i]))
    if(i==1) dfm1<-df_out
...
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfoutput/",name_folders[2],"/",m2[i]))
  if(i==1) dfm2<-df_out
  if(i>1) dfm2<-rbind(dfm2, df_out)
}


write.csv(dfm1, file="data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfm1.csv")  # these pvalues are NOT adjusted
write.csv(dfm2, file="data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/dfm2.csv")  # these pvalues are NOT adjusted
...
m1<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/models/",name_folders[1],"/"))
m2<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/models/",name_folders[2],"/"))

fxn_sort<-function(x)
  {return( paste( sort(strsplit(x, split="_")[[1]]) ,collapse ="_")) }

library(letsR)
list.files("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/")


pdf("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/MoransI_nbhoodV1_forS.pdf", height = 10, width = 10)
par(mfrow=c(4,4))
par(mar=c(4,5,0.5,0.5))

for(i in 1: length(m1))
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/models/",name_folders[1],"/",m1[i]))
...
pdf("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/MoransI_nbhoodV210_forS.pdf", height = 10, width = 10)
par(mfrow=c(4,4))
par(mar=c(4,5,0.5,0.5))

for(i in 1: length(m2))
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_EDGE/models/",name_folders[2],"/",m2[i]))
  matx<-as.matrix(residuals(model))
  socs<-paste(model@frame$soc1,model@frame$soc2,sep="_")
...
model <- glmer(share ~ RC1 + RC2 + RC3 + RC4 + RC5 + 
                     (1 | soc1) + (1|soc2), data = dfmSC, family = binomial, 
                   control = glmerControl(optimizer = "bobyqa",optCtrl=list(maxfun=2e5)))
# don't save model - confusing for rest of analyses
vifm<-vif(model) # futile now since RPCAs
summary(model)->sumM
sumM$coefficients-> sumMc
nagelkerke(model)->nagel
...
df_out
# write.csv(df_out, file=paste0("manuscript/Tables_EDGE/TableS4c_", trait_of_interest,".csv")) 
  # modify table name - the a/c from name depends on nbhood

load(file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmEDGERPCA/",name_folder,"/","RCtraits_","_", trait_of_interest,".rds"))
...
# write.csv(RCtraits, file=paste0("manuscript/Tables_EDGE/TableS2c_", trait_of_interest,".csv"))
  # modify table name - the a/c from name depends on nbhood