...
for(i in 1: length(traits_of_interest))
  ...
  load("data/predictors/preds3april/phylogeny/lfammat.rda") # share family
  rownames(lfammat) -> l2
  ...
  lfammat<-lfammat[soclist,soclist]
  ...
  socnames<-extract_socid_model_vectors(share_ctraits_mat, subset=T) ; save(socnames, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","socnames_pertrait_",trait_of_interest,".rda"))
  ctraits_values<-extract_variable_model_vectors(share_ctraits_mat, subset=T); save(ctraits_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","ctraits_pertrait_",trait_of_interest,".rda"))
  
  sharefam_values<-extract_variable_model_vectors(lfammat, subset=T); save(sharefam_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","sharefam_values_pertrait_",trait_of_interest,".rda"))
  edTemp_values<-extract_variable_model_vectors(edTemp, subset=T); save(edTemp_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","edTemp_values_",trait_of_interest,".rda"))
  edXeric_values<-extract_variable_model_vectors(edXeric, subset=T); save(edXeric_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","edXeric_values_",trait_of_interest,".rda"))
  
  gdist_values<-extract_variable_model_vectors(mdist_matrix_distGeo, subset=T); save(gdist_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmFamID/","gdist_valuesGeo_pertrait_",trait_of_interest,".rda"))
  print(i) #i
