...
# reverse each nb with its soci
soci1<-unique(dfiEA$nb)

for(i in 1:length(soci1))
  {   socO<-soci1[i]
  socDs<-as.character(dfiEA[dfiEA$nb%in%socO,]$soci)
  ...
  save(result, file=paste0("data/Re_analysisGEbarriers/P1_elevation_nb/elevation_", socO,".rds"))
