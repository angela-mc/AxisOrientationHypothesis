...
for(i in 1: length(traits_of_interest))
  ...
  #load(file="data/predictors/preds3april/phylogeny/Jagercophenetic_distAfterCalib.rda") # Jager phy D
  load(file="data/predictors/preds3april/phylogeny/Jagercophenetic_distAfterCalib_newtree.rda") # if new tree is used - all is saved in _newtree folders
  ...
  socnames<-extract_socid_model_vectors(share_ctraits_mat, subset=T) ; save(socnames, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","socnames_pertrait_",trait_of_interest,".rda"))
  ctraits_values<-extract_variable_model_vectors(share_ctraits_mat, subset=T); save(ctraits_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","ctraits_pertrait_",trait_of_interest,".rda"))
  
  pd_values<-extract_variable_model_vectors(cophenetic_dist, subset=T); save(pd_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","pd_values_pertrait_",trait_of_interest,".rda"))
  edTemp_values<-extract_variable_model_vectors(edTemp, subset=T); save(edTemp_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","edTemp_values_",trait_of_interest,".rda"))
  edXeric_values<-extract_variable_model_vectors(edXeric, subset=T); save(edXeric_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","edXeric_values_",trait_of_interest,".rda"))
  
  gdist_values<-extract_variable_model_vectors(mdist_matrix_distGeo, subset=T); save(gdist_values, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJager/","gdist_valuesGeo_pertrait_",trait_of_interest,".rda"))
