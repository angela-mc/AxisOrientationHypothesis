...
for(k in 1: 2)
  ...
  for(j in 1: length(traits_of_interest))
    if(!(k==2 & j==34)) # errors, no convergence
      ...
      if(!paste(trait_of_interest, name_folder)%in%listw & paste0("dfmS2_", "_",trait_of_interest,".csv") %in% list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJagerRPCA/",name_folder)))  
        # for now the ones that 100% work
        {read.csv(file =paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJagerRPCA/",name_folder,"/","dfmS2_", "_",trait_of_interest,".csv"), stringsAsFactors = F)->dfm
          ...
          save(model, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/models/",name_folder,"/model_",trait_of_interest,"_",100,".rds"))
          ...
          df_out<-data.frame("Trait"=trait_of_interest, "Name_tr"=vars[vars$id%in%trait_of_interest,]$title, 
              ...
              save(df_out, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folder,"/model_",trait_of_interest,".rds"))
              
       # replace the names of RCs with real predictor name
       load(file=paste0("data/Re_analysisGEbarriers/P1_datamodel/dfmJagerRPCA/",name_folder,"/","RCtraits_","_", trait_of_interest,".rds")) # RCtraits
       RCs<-character()
       ...
       save(df_out, file=paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folder,"/dfoutNamesPred_",trait_of_interest,".rds"))
...
# ###########################
# #### big df ############### remove k=2 & j= 32 for now as it didn't converge
# ###########################

name_folders<-c("nbhoodV1","nbhoodV210") ##  the analysis will be separate for each nbhood structure ie 67 traits 

m1<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folders[1],"/"))
m1<-m1[grep("dfoutNamesPred", m1)]
m2<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folders[2],"/"))
m2<-m2[grep("dfoutNamesPred", m2)]

rm(dfm1)
for(i in 1:length(m1))
  { load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folders[1],"/",m1[i]))
    if(i==1) dfm1<-df_out
...
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfoutput/",name_folders[2],"/",m2[i]))
  if(i==1) dfm2<-df_out
  if(i>1) dfm2<-rbind(dfm2, df_out)
}

# remove the ones that didn t converge
rem<-which(as.character(dfm2$Trait)%in%"EA045")
dfm2<-dfm2[-rem,]

write.csv(dfm1, file="data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfm1.csv")  # these pvalues are NOT adjusted
write.csv(dfm2, file="data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/dfm2.csv")  # these pvalues are NOT adjusted

 
# ###########################
# #### Moran s I ############ remove k=2 & j= 32 for now as it didn't converge
# ###########################
...
m1<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/models/",name_folders[1],"/"))
m2<-list.files(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/models/",name_folders[2],"/"))

fxn_sort<-function(x)
  {return( paste( sort(strsplit(x, split="_")[[1]]) ,collapse ="_")) }

library(letsR)
list.files("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/")


pdf("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/MoransI_nbhoodV1_forS.pdf", height = 10, width = 10)
par(mfrow=c(4,4))
par(mar=c(4,5,0.5,0.5))

for(i in 1: length(m1))
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/models/",name_folders[1],"/",m1[i]))
...
pdf("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/MoransI_nbhoodV210_forS.pdf", height = 10, width = 10)
par(mfrow=c(4,4))
par(mar=c(4,5,0.5,0.5))

for(i in 1: length(m2))
{ load(paste0("data/Re_analysisGEbarriers/P1_datamodel/glmerRCA_Jager/models/",name_folders[2],"/",m2[i]))
  matx<-as.matrix(residuals(model))
  socs<-paste(model@frame$soc1,model@frame$soc2,sep="_")
...
length(unique(c(m1,m2))) # 52
  # EA045 did not converge in m2 but it did in m1
